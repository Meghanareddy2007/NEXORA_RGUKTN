...
const DEMO_ACCOUNTS: {
  ...
  {
    dept: "TMS",
    label: "Track Maintenance System",
    username: "tms_user",
    password: "tms123",
    tone: "success",
  },
  {
    dept: "SMMS",
    label: "Signal & Telecom Maintenance",
    username: "smms_user",
    password: "smms123",
    tone: "warning",
  },
  {
    dept: "TRACTION",
    label: "Traction Distribution Maintenance",
    username: "traction_user",
    password: "traction123",
    tone: "info",
  },
  {
    dept: "COA",
    label: "Corridor & Block Availability",
    username: "coa_user",
    password: "coa123",
    tone: "violet",
  },
...
> = {
  success: {
    active: "bg-success text-[#062014]",
    dot: "bg-success",
  },
  warning: {
    active: "bg-warning text-[#241703]",
    dot: "bg-warning",
  },
  info: {
    active: "bg-info text-[#031521]",
    dot: "bg-info",
  },
  violet: {
    active: "bg-violet-400 text-[#150a28]",
    dot: "bg-violet-400",
  },
  primary: {
...
export default function LoginPage() {
  ...
  const active =
    DEMO_ACCOUNTS.find((a) => a.dept === selectedDept) ??
    DEMO_ACCOUNTS[0];

  const tone = TONE_CLASSES[active.tone];

  function selectDept(
    acct: (typeof DEMO_ACCOUNTS)[number]
  ) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
       ...
       *
       * IMPORTANT:
       * We use the department returned by the backend
       * instead of trusting the selected department button.
       */
       ...
       *
       * TMS       -> TMS dashboard
       * TRACTION  -> TDMS dashboard
       * SMMS      -> SMMS dashboard
       * COA       -> COA dashboard
       * ADMIN     -> Main dashboard
       */

      if (user.department === "TMS") {
        router.push("/tms");
      } else if (user.department === "TRACTION") {
        router.push("/tdms");
      } else if (user.department === "SMMS") {
        router.push("/smms");
      } else if (user.department === "COA") {
        router.push("/coa");
      } else if (user.department === "ADMIN") {
        router.push("/");
      } else {
        setError(
          "Your account does not have access to a dashboard."
        );
      }
    } catch (err: any) {
      setError(
    ...
    <div className="relative flex min-h-screen overflow-hidden bg-background">
      ...
      <div className="relative hidden w-[58%] flex-col justify-end overflow-hidden lg:flex">
        ...
        <div className="relative z-10 max-w-2XL px-11 pb-4">
          ...
          <p
            className="login-fade-in mt-3.5 max-w-full text-sm leading-relaxed text-muted-foreground"
            style={{ animationDelay: "0.18s" }}
          >
            A single console for Track, Signal &amp; Telecom,
            Traction and Corridor Availability teams —replacing
            spreadsheets and phone calls with an optimized,
            conflict-free block calendar.
          </p>

          <div
            className="login-fade-in mt-5 flex gap-4 font-mono text-[11.5px] text-muted-foreground"
            ...
            {DEMO_ACCOUNTS.map((acct) => (
              <button
                ...
                className={`rounded-md py-2 text-[11px] font-medium font-mono transition-colors ${
                  selectedDept === acct.dept
                    ? TONE_CLASSES[acct.tone].active
